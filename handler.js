        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              let wel = `https://hardianto-chan.herokuapp.com/api/welcome3?profile=${pp}&name=${encodeURIComponent(this.getName(user))}&bg=https://i.ibb.co/r7DVt61/a8273e1be3af.jpg&namegb=${encodeURIComponent(this.getName(jid))}&member=${encodeURIComponent(groupMetadata.participants.length)}`
              let lea = `https://hardianto-chan.herokuapp.com/api/goodbye3?profile=${pp}&name=${encodeURIComponent(this.getName(user))}&bg=https://i.ibb.co/r7DVt61/a8273e1be3af.jpg&namegb=${encodeURIComponent(this.getName(jid))}&member=${encodeURIComponent(groupMetadata.participants.length)}`

              this.sendFile(jid, action === 'add' ? wel : lea, 'pp.jpg', text, { 
key: {
fromMe: false,
...
Hapus Apa Itu Hayo :v
`.trim(), '', 'DISABLE DELETE', '#off antidelete', {
      quoted: m.message,
      contextInfo: {
    ...
    async onCall(json) {
        ...
        await this.sendMessage(from, 'Bot Tidak Menerima Panggilan. Maaf Anda akan diblokir, silakan hubungi pemilik bot ini untuk membuka blokir ini wa.me/628988293493', MessageType.extendedText)
...
global.dfail = (type, m, conn, unreg, ftroli) => {
  let msg = {
    ...
    premium: '_FITUR INI KHUSUS UNTUK PREMIUM KETIK #premium UNTUK MEMBELI PREMIUM_',
    group: '_*「 ! 」GROUP ONLY*_',
    private: '_*「 ! 」PRIVATE ONLY*_',
    admin: '_*「 ! 」ADMIN GROUP ONLY*_',
    botAdmin: '_*「 ! 」 BOT ADMIN ONLY*_',    
    nsfw : 'NFSW Not Active'
  }[type]
  if (msg) return conn.reply(m.chat, msg, 'conversation', { quoted: m, contextInfo: { externalAdReply :{
sourceUrl: 'https://instagram.com/ftditzzxy',
mediaType: 2,
...
const ftoko = { key : { fromMe: false, participant : `0@s.whatsapp.net`, remoteJid: 'status@broadcast' },message: {orderMessage: { message: `BELUM TERDAFTAR`,thumbnail: fs.readFileSync("./src/logo.jpg")}}}
  conn.sendButton(m.chat, `_*Hai Kak ${conn.getName(m.sender)} Kamu Belum Terdaftar Dalam Database YukiBot.*_\n_*Silahkan Register Terlebih Dahulu*_\n\n_*Jika Kamu Tidak Paham Ketik #daftar namamu.umurmu*_`, '', 'REGISTER', '#registered', { quoted: ftoko })
